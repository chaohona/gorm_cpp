...
int GORM_Socket::CreateAndListen(GORM_Log *logHandle, const char *szIp, uint16 usPort, int iBackLog)
{
    int iFD = socket(AF_INET, SOCK_STREAM, 0);
    if (iFD <= 0)
    {
#ifndef _WIN32
        GORM_CUSTOM_LOGE(logHandle, "create socket failed:%d, errmsg:%s", errno, strerror(errno));
#endif
		return iFD;
    }
    ...
    {
#ifndef _WIN32
        GORM_CUSTOM_LOGE(logHandle, "set resuse addr failed:%d,errmsg:%s", errno, strerror(errno));
#endif
		return GORM_ERROR;
    }
    ...
    if(iRet == -1) {
#ifndef _WIN32
        GORM_CUSTOM_LOGE(logHandle, "bind failed:%d, errmsg:%s", errno, strerror(errno));
#endif
		return iRet;
    }
    if (iBackLog < 1)
    ...
    if(iRet == -1) {
#ifndef _WIN32
        GORM_CUSTOM_LOGE(logHandle, "listen failed:%d, errmsg:%s", errno, strerror(errno));
#endif
		return iRet;
    }
    ...
    {
#ifndef _WIN32
        GORM_CUSTOM_LOGE(logHandle, "set socket nonblock failed:%d,errmsg:%s", errno, strerror(errno));
#endif
		return GORM_ERROR;
    }
    GORM_CUSTOM_LOGD(logHandle, "create listen socket success, port:%d, fd:%d", usPort, iFD);
    return iFD;
}
